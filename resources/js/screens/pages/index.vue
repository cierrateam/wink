  ...
  <div>
    <page-header>
      <template slot="right-side">
        <router-link :to="{name:'page-new'}" class="py-1 px-2 btn-primary text-sm">New Page</router-link>
      </template>
    </page-header>
    ...
    <div class="container">
      ...
      <div v-if="ready && entries.length == 0 && !isFiltered">Keine Seiten gefunden. Starte damit
        <router-link
          :to="{name:'page-new'}"
          class="no-underline text-primary hover:text-primary-dark"
        >eine Seite zu erstellen.</router-link>.
      </div>

      <div v-if="ready && entries.length == 0 && isFiltered">No pages matched the given search.</div>
        ...
        >
          <div class="py-4" :title="entry.title">
            <h2 class="text-xl font-semibold mb-3">
              <router-link
                :to="{name:'page-edit', params:{id: entry.id}}"
                class="no-underline text-text-color"
              >{{truncate(entry.title, 68)}}</router-link>
            </h2>

            <p class="mb-3">{{truncate(entry.body.replace(/(<([^>]+)>)/ig,""), 100)}}</p>

            <small class="text-light">
              Geändert {{timeAgo(entry.updated_at)}}
              — Erstellt {{timeAgo(entry.created_at)}}
            </small>
          </div>
        </div>
